...
class RepoLabel(Base):
    """Multi-dimensional labels for repositories."""
    __tablename__ = "repo_labels"
    
    id = Column(Integer, primary_key=True, index=True)
    repo_id = Column(Integer, ForeignKey("repos.id"), nullable=False, index=True)
    label_type = Column(String(50), nullable=False, index=True)  # category, quality, technical, community, discovery
    ...
    __table_args__ = (
        Index("idx_repo_label_type", "repo_id", "label_type"),
    )


class RepoInsight(Base):
    """Insightfulness metrics for repositories."""
    ...
    production_use_score = Column(Float, nullable=False)
    community_impact = Column(Float, nullable=False)
    technical_depth = Column(Float, nullable=False)
    total_insightfulness = Column(Float, nullable=False, index=True)
    computed_at = Column(DateTime, server_default=func.now())


class SocialSignals(Base):
    """Social signals and real-world usage metrics for repositories."""
    ...
    reddit_subreddits = Column(JSON, default=list)  # List of subreddits where mentioned
    
    # Hacker News metrics
    hn_mentions = Column(Integer, default=0)
    hn_points = Column(Integer, default=0)
    hn_comments = Column(Integer, default=0)
    
    # Stack Overflow metrics
    stackoverflow_questions = Column(Integer, default=0)
    stackoverflow_views = Column(Integer, default=0)
    stackoverflow_score = Column(Integer, default=0)
    
    # Package manager metrics (npm, pypi, etc.)
    npm_weekly_downloads = Column(Integer, default=0)
    pypi_weekly_downloads = Column(Integer, default=0)
    package_name = Column(String(255))  # Package name if published
    
    # Twitter/X metrics (if available via API)
    twitter_mentions = Column(Integer, default=0)
    
    # Aggregated social score (0-100)
    social_score = Column(Float, default=0.0, index=True)
    ...
    __table_args__ = (
        ...
        Index("idx_social_updated", "last_updated"),
    )
